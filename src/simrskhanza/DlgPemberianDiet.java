...
public class DlgPemberianDiet extends javax.swing.JDialog {
    ...
    public DlgPemberianDiet(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, new Object[]{
            "No.Rawat", "Nama Pasien", "Kamar", "Tanggal", "Waktu", "Jam", "Diet", "Dokter PJ", "Diagnosa", "Kode Kamar", "Kode Diet"
        }) {
            @Override
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (TabRawat.getSelectedIndex() == 0) {
            if (TNoRw.getText().trim().equals("") || TPasien.getText().trim().equals("")) {
                ...
                if (Sequel.menyimpantf("detail_beri_diet", "'" + TNoRw.getText() + "','"
                        + Kamar.getText() + "','"
                        + Valid.SetTgl(DTPTgl.getSelectedItem() + "") + "','"
                        + WaktuDiet.getText() + "','"
                        + KdDiet.getText() + "'", "data") == true) {

                    // Ambil data dokter dari tabel dpjp_ranap
                    String nmDokter = "";
                    try {
                        String query = "SELECT dokter.nm_dokter "
                                + "FROM dpjp_ranap INNER JOIN dokter "
                                + "ON dpjp_ranap.kd_dokter = dokter.kd_dokter "
                                + "WHERE dpjp_ranap.no_rawat = ? "
                                + "ORDER BY dpjp_ranap.no_rawat DESC LIMIT 1";
                        ...
                        System.out.println("Error getting DPJP RANAP: " + e);
                    }

                    // Ambil diagnosa
                    String diagnosa = Sequel.cariIsi(
                            "select kamar_inap.diagnosa_awal from kamar_inap "
...
//        }
        ...
        if (TabRawat.getSelectedIndex() == 0) {
            ...
            String kdKamar = tabMode.getValueAt(row, 9).toString();
            String tanggal = tabMode.getValueAt(row, 3).toString();
            String waktu = tabMode.getValueAt(row, 4).toString();
            String kdDiet = tabMode.getValueAt(row, 10).toString();

            // Konfirmasi UNTUK HAPUS
            int jawab = JOptionPane.showConfirmDialog(null,
                    "Hapus data diet pasien ini?",
                    "Konfirmasi", JOptionPane.YES_NO_OPTION);

            if (jawab == JOptionPane.YES_OPTION) {
                boolean success = Sequel.queryutf(
                        "DELETE FROM detail_beri_diet WHERE "
                        + "no_rawat='" + noRawat + "' AND "
                        + "kd_kamar='" + kdKamar + "' AND "
                        + "tanggal='" + tanggal + "' AND "
                        + "waktu='" + waktu + "' AND "
                        + "kd_diet='" + kdDiet + "'"
                );
                ...
                if (success) {
                    ...
                    try {
                        String sql = "DELETE FROM detail_beri_diet WHERE "
                                + "no_rawat=? AND kd_kamar=? AND tanggal=? AND waktu=? AND kd_diet=?";
                        PreparedStatement ps = koneksi.prepareStatement(sql);
                        ps.setString(1, noRawat);
                        ps.setString(2, kdKamar);
                        ps.setString(3, tanggal);
                        ps.setString(4, waktu);
                        ps.setString(5, kdDiet);
    ...
    public void tampil() {
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + "detail_beri_diet.kd_kamar, detail_beri_diet.kd_diet, "
                    // AMBIL DOKTER DPJP RANAP
                    + "dpjp_ranap.kd_dokter AS dpjp_kd_dokter, dokter.nm_dokter AS dpjp_nama "
                    ...
                    + "INNER JOIN jam_diet_pasien ON detail_beri_diet.waktu = jam_diet_pasien.waktu "
                    // JOIN TABEL DPJP
                    + "LEFT JOIN dpjp_ranap ON dpjp_ranap.no_rawat = detail_beri_diet.no_rawat "
                    + "LEFT JOIN dokter ON dpjp_ranap.kd_dokter = dokter.kd_dokter "
                    + "WHERE detail_beri_diet.tanggal BETWEEN ? AND ? "
                    + "AND detail_beri_diet.waktu LIKE ? "
                    + "AND bangsal.nm_bangsal LIKE ? "
                    + (TCari.getText().trim().equals("") ? ""
                    : "AND (detail_beri_diet.no_rawat LIKE ? "
...
//        }
        if (tbDataDiet.getSelectedRow() != -1) {
            ...
            TNoRw.setText(tabMode.getValueAt(row, 0).toString());
            TPasien.setText(tabMode.getValueAt(row, 1).toString());
            Ruang.setText(tabMode.getValueAt(row, 2).toString());
            WaktuDiet.setText(tabMode.getValueAt(row, 4).toString());
            JamDiet.setText(tabMode.getValueAt(row, 5).toString());
            NmDiet.setText(tabMode.getValueAt(row, 6).toString());

            // Pastikan mengambil dari kolom yang benar
            Kamar.setText(tabMode.getValueAt(row, 9).toString()); // Kolom kd_kamar (hidden)
            KdDiet.setText(tabMode.getValueAt(row, 10).toString()); // Kolom kd_diet (hidden)

            // Set tanggal dengan format yang benar
            String tanggalStr = tabMode.getValueAt(row, 3).toString();
            try {
                Valid.SetTgl(DTPTgl, tanggalStr);
            } catch (Exception e) {
                System.out.println("Error setting tanggal: " + e);
                // Fallback jika format tanggal bermasalah
                DTPTgl.setDate(new Date());
            }
