...
public final class RMPenilaianAwalKeperawatanRanapBayiAnak extends javax.swing.JDialog {
    ...
    */
    ...
    private void isRawat() {
        ...
        System.out.println("===== MASUK isRawat() =====");
        ...
        int cekIGD = Sequel.cariInteger(
            "select count(no_rawat) from penilaian_medis_igd where no_rawat=?",
            noRawat
        );
        if (cekIGD > 0) {

            Suhu.setText(Sequel.cariIsi("select suhu from data_triase_igd where no_rawat=?", noRawat));
            TD.setText(Sequel.cariIsi("select tekanan_darah from data_triase_igd where no_rawat=?", noRawat));
            Nadi.setText(Sequel.cariIsi("select nadi from data_triase_igd where no_rawat=?", noRawat));
            RR.setText(Sequel.cariIsi("select pernapasan from data_triase_igd where no_rawat=?", noRawat));

            RPK.setText(Sequel.cariIsi("select rpk from penilaian_medis_igd where no_rawat=?", noRawat));
            RPO.setText(Sequel.cariIsi("select rpo from penilaian_medis_igd where no_rawat=?", noRawat));
            RPS.setText(Sequel.cariIsi("select keluhan_utama from penilaian_medis_igd where no_rawat=?", noRawat));
            RPD.setText(Sequel.cariIsi("select rpd from penilaian_medis_igd where no_rawat=?", noRawat));
        }

        // ================== CEK DARI RALAN BAYI ==================
        int cekRalan = Sequel.cariInteger(
            "select count(no_rawat) from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
            noRawat
        );
        ...
        javax.swing.SwingUtilities.invokeLater(new Runnable() {
            public void run() {
                ...
                Suhu.setText(Sequel.cariIsi(
                    "select suhu from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                TD.setText(Sequel.cariIsi(
                    "select td from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                Nadi.setText(Sequel.cariIsi(
                    "select nadi from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                RR.setText(Sequel.cariIsi(
                    "select rr from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                TB.setText(Sequel.cariIsi(
                    "select tb from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                BB.setText(Sequel.cariIsi(
                    "select bb from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                RPK.setText(Sequel.cariIsi(
                    "select rpk from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                RPO.setText(Sequel.cariIsi(
                    "select rpo from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                RPS.setText(Sequel.cariIsi(
                    "select keluhan_utama from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));

                RPD.setText(Sequel.cariIsi(
                    "select rpd from penilaian_awal_keperawatan_ralan_bayi where no_rawat=?",
                    noRawat
                ));
            }
        });
